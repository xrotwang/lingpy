...
class MSA(Multiple):
    ...
    """
    def __init__(self, initdict, **keywords):
        util.setdefaults(
            keywords,
        ...
        if not isinstance(initdict, dict):
            initdict = read_msa(initdict, **keywords)
        for key in initdict:
            if key not in 'seqs':
...
class Alignments(Wordlist):
    ...
    def reduce_alignments(self, ref='cogid'):
        ...
        if 'alignment' not in self.header:
            raise ValueError(
                'No alignments found in your data. You'
    ...
    def align(self, **keywords):
        ...
        with util.ProgressBar('ALIGNMENTS', len(self.msa[kw['ref']])) as progress:
            for key, value in sorted(
                ...
                self._meta['msa'][kw['ref']][key]['stamp'] = \
                    rcParams['align_stamp'].format(
...
def get_consensus(
    ...
    def rep_weights(char1, char2, prosody):
        if char1 == char2:
            return 0
        if char1 == '-':
            return 0.1 if char2 in charset else 0.8
        if char2 == "-":
            return 0.3 if char1 in charset else 0.1
        return 0.1
    ...
    # match
        ...
        # FIXME: this operates on the non-transposed alm_matrix
